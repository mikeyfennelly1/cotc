...
[source,bash]
----
git clone --recurse-submodules git@github.com:mikeyfennelly1/cotc.git
----
...
| Collector
| Runs directly via link:https://github.com/mikeyfennelly1/cotccollector/blob/main/start.sh[`./start.sh`], which invokes `./gradlew bootRun` to run the JAR in-process.
| Runs via Docker Compose, exposed through NGINX.

| web-app
| Runs directly via link:https://github.com/mikeyfennelly1/cotcsubscriber/blob/main/start.sh[`./start.sh`], which invokes `./gradlew bootRun` to run the JAR in-process.
| Runs via Docker Compose, exposed through NGINX.
